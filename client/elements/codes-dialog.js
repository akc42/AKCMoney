...
class CodesDialog extends LitElement {
        ...
        <div class="listcontainer">
          ${cache(this.codes.map((code, i) => html`
            ...
            <button type="button" role="menuitem" 
              ...
              <span>${code}</span>
              ${code === this.code ? html`<span><material-icon class="codes-icon">check_box</material-icon></span>` : ''}
            </button>
          `))}        
        </div>
      </dialog-box>
  ...
  _closing(e) {
    e.stopPropagation();
    this.dialog.positionTarget.dispatchEvent(new CustomEvent('value-changed', { bubbles: true, composed: true, detail: this.code })); //tell the outside world we have a value
    this.eventLocked = false;
  }
  _gotRequest(e) {
    e.stopPropagation();
