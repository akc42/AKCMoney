...
(function() {
  ...
  function startUp (http, serverDestroy,Router, finalhandler, Responder, logger, db, bcrypt, versionPromise) {
    try {
      /*
...
document.cookie = '${serverConfig.trackCookie}=${token}; expires=0; Path=/'; 
      ...
      */
      ...
      versionPromise.then(info => 
        logger('app', `Release ${info.version} of money Server Operational on Port:${
  ...
  if (!module.parent) {
    ...
    startUp(http, serverDestroy, Router, finalhandler, Responder, logger, db, bcrypt, versionPromise);
