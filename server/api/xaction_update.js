...
const logger = Logger('xactionupdate', 'error');
  ...
  FROM items WHERE t.id = ?`);
    ...
    if (tid === 0) {
      ...
      if (altAccount !== null) {
        ...
        console.nullIfZeroLength('have other account, so we need to worry about currency of', acurrency);
        if (acurrency === params.currency) {
          ...
          if (params.currency === dc) {
            ...
            const {rate:arate} = db.get`SELECT rate FROM Currency WHERE name = ${acurrency}`??{rate:1};
            rate = arate
          } else if (acurrency === dc) {
            const {rate:drate} = db.get`SELECT rate FROM Currency WHERE name = ${params.currency}`??{rate:1};
            rate = 1/drate
          } else {
            const {rate:aprate} = db.get`SELECT rate FROM Currency WHERE name = ${acurrency}`??{rate:1};
            const {rate: parate} = db.get`SELECT rate FROM Currency WHERE name = ${params.currency}`??{rate:1};
            rate = aprate/parate;
          }
          console.log('completely different currency in this account so rate', rate);
      ...
      console.log('made new transaction with id = ', lastInsertRowid);
      if (params.cleared !== undefined && ((currency === params.currency && nullOrAmount(params.amount) !== null) ||
        ...
        console.log('updated account',params.account,'with new balance change of', balanceChange);
      ...
      if (version === Number(params.version)) {
        ...
        if ((params.cleared === undefined && (srcclear === 1 || dstclear === 1)) || 
          ...
          console.log('transaction update is possibly changing at least one of the account balances');
          ...
          if (balanceChange !== 0) {
            ...
            responder.addSection('balance', balance + balanceChange);
            responder.addSection('bversion', bversion + 1);
            console.log('updated account', params.account, 'with balance change of', balanceChange);
          }
        }
        ...
        if (prevAltAccount !== null && ((prevAltAccount === src && srcclear === 1) || (prevAltAccount === dst && dstclear === 1)) || 
          ...
          let balanceChange = 0;  
          console.log('either', prevAltAccount, 'or', newAltAccount, 'balance will change - lets calculate'); 
          ...
          if (balanceChange !== 0) {
            console.log('previous account balance needs update', prevAltAccount);
        ...
        if (newAltAccount !== null && newAltAccount !== prevAltAccount && params.cleared !== undefined) { 
          ...
          if (ncurrency === params.currency) {
            ...
            //currency isn't anything we have an amount for, so we are going to have to calculate one for the account
            // so we get the default currency first
            const {dc} = db.get`SELECT name AS dc FROM currency WHERE priority = 0`??{dc:0}
            let rate;
            if (params.currency === dc) {
              //transaction is in default currency, so we need to get rate of newAltAccount
              const {rate: nrate} = db.get`SELECT rate FROM Currency WHERE name = ${ncurrency}`??{rate:1};
              rate = nrate;
            } else if (ncurrency === dc) {
              const {rate: prate} = db.get`SELECT rate FROM Currency WHERE name = ${params.currency}`??{rate:1};
              rate = 1/prate
            } else {
              const {rate: nprate} = db.get`SELECT rate FROM Currency WHERE name = ${ncurrency}`??{rate:1};
              const {rate: pnrate} = db.get`SELECT rate FROM Currency WHERE name = ${params.currency}`??{rate:1};
              rate = nprate/pnrate;
            }
            balanceChange = Math.round(rate * nullOrAmount(params.amount)) * (newAltAccount === params.src? -1:1)
          }
          if (balanceChange !== 0) {
            console.log('new Alt account balance needs update', newAltAccount);
        ...
        if (params.account === params.src) {
          console.log('type src - source account', src, 'account', params.account);
          if (params.account !== src) swapSrcDst.run(tid); //we have to swap first
          if (params.type === 'move' && dst === null && params.dst.length > 0) {
            console.log('Previous dst was null, but we moved to it, so make src', params.dst,'and make dst null (later update)');
          ...
          console.log('type dst - dst account', dst, 'account', params.account);
          if (params.account !== dst) swapSrcDst.run(tid); //we have to swap first
          if (params.type === 'move' && src === null && params.src.length > 0) {
            console.log('Previous src was null, but we moved to it, so make dst', params.src, 'and make src null (later update)');
          ...
          logger('Invalid Account:', params.account, ',source:', params.src, ',destination:', params.dst);
        ...
        console.log('tid', tid, 'params.account', params.account);
        const transaction = getUpdatedXaction.get(tid, params.account)
        console.log('transaction', JSON.stringify(transaction))
        responder.addSection('transaction', transaction);
        responder.addSection('status', 'OK');
      } else {
        logger('params version', params.version, 'database version', version,'tid', tid);
